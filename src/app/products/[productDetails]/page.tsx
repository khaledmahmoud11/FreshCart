...
import { Check, Eye, Repeat2Icon, Share2, ShoppingBasket, Star,  Truck,  Zap } from 'lucide-react';
...
import AddToCartBtn from '@/app/Components/Commens/AddToCartBtn'
import Image from 'next/image'
import AddToWishBtn from '@/app/Components/Commens/AddToWishBtn'
  ...
  <>
    ...
    <Tabs defaultValue="account" className=" container px-4 my-5">
                  ...
                  <TabsContent value="reviews">
                    ...
                    <CreateReview productIdD={product._id} />
                    ...
                    <div className=" col-span-1 relative hover:shadow-md hover:-translate-y-1.5 duration-400 transition-all rounded-2xl ">
                      {item.priceAfterDiscount && 
                        <div className="absolute rounded-lg text-sm bg-red-500 font-bold text-white px-2 py-1 top-5 left-5">
                          {Math.round(((item.priceAfterDiscount-item.price)/item.price)*100)} %
                        </div>
                      }
                      
                      <div className="absolute top-5 right-0  space-y-4">
                        <AddToWishBtn productId={item._id} />
                      ...
                      <Card >
                        <Link href={`/products/${item._id}`}>
                          ...
                          <CardHeader>
                            ...
                            <CardTitle title={item.title} className="text-xl font-bold">
                            ...
                            <div className="flex items-center gap-2">
                              <div className="flex items-center gap-2">
                                {[0,1,2,3,4].map((index)=>{
                                  const filledStar = index < Math.floor(item.ratingsAverage)
                                  return<React.Fragment key={index}>
                                    <Star size={16} className= {filledStar ?"text-yellow-500 fill-yellow-500 ":"text-gray-500 fill-gray-500" }/>
                                  </React.Fragment>
                              ...
                              <p className='text-gray-400'> {item.ratingsAverage} </p>
                              <p className='text-gray-400'>( {item.ratingsQuantity} )</p>
                            </div>
                          </CardHeader>
                        </Link>
                        <CardContent className="flex items-center justify-between">
                          {item.priceAfterDiscount && <p className='text-xl font-bold text-green-600' >{item.priceAfterDiscount} Egp</p> }
                          <p className={` ${item.priceAfterDiscount ? "text-gray-600 text-md line-through" : "text-lg"}  font-bold`}>{item.price} Egp</p>
                          <div >
